# Component: NAAS Spec 1 — Chunk 3: app/routes.py (APIRouter + composition-root wiring)
# Mode: TDD — ALL tests in this file MUST FAIL until the implementer creates:
#   services/event-ingestion/app/routes.py   (new APIRouter with three endpoints)
#   services/event-ingestion/app/main.py     (modified: mounts router, removes stub /health)
#
# What these tests validate (spec §§3.3, 5.6, 5.7):
#   (a) Endpoint surface: exactly POST /events/ingest, POST /events/bulk, GET /health —
#       nothing else (excluding FastAPI's /openapi.json, /docs, /redoc auto-routes).
#   (b) Single ingest happy path: POST /events/ingest returns 202 with body
#       {"id": "<uuid>", "status": "accepted"}. Route delegates entirely to the service;
#       it contains no dual-write logic of its own.
#   (c) Bulk happy path: POST /events/bulk with a bare JSON array of 3 events returns
#       202 with {"accepted": 3, "event_ids": [3 uuids], "status": "accepted"}.
#   (d) Bulk size validation: empty array or array > 5000 elements → 422 without
#       calling the service.
#   (e) Field validation rejections (422, service not called): invalid client_ip,
#       unknown protocol.
#   (f) Publish-after-commit resilience: when the service's ingest_one propagates a
#       publish failure (simulating a service configured with a broken publisher but
#       a committed PG write), the route still returns 202. NOTE: IngestionService
#       already swallows publish failures internally (spec §5.5 / Chunk 2), so this
#       test drives a fake service that mimics committed-but-publish-failed behavior
#       and still returns the UUID.
#
# Override strategy:
#   We inject a FakeIngestionService via app.dependency_overrides, keyed on whatever
#   callable the implementer uses as the FastAPI dependency that yields / returns an
#   IngestionService. The test uses a sentinel pattern: if the override is not in
#   effect (i.e., the routes were never mounted), the real dependency would attempt
#   a DB connection and fail — that failure is also a correct TDD "fails initially".
#
#   IMPORTANT: routes.py will define a FastAPI `Depends(get_ingestion_service)` (or
#   similar). We override via the function reference. If the implementer chooses a
#   different dependency name, the override must be updated. We use
#   `app.dependency_overrides[get_ingestion_service] = lambda: fake_service` to
#   avoid any database or Redis connection. This is the idiomatic FastAPI approach
#   documented in https://fastapi.tiangolo.com/advanced/testing-dependencies/.
#
# No live infrastructure required: all DB/Redis paths are overridden.
#
# sys.path strategy: identical to prior chunk tests (repo-root discovery, inject
#   shared/ and services/event-ingestion/).
...
# ---------------------------------------------------------------------------
# Repo-root discovery and sys.path injection (identical pattern to prior chunks)
# ---------------------------------------------------------------------------
...
def _import_app_and_service_dep():
    ...
    """
    # This import will raise ImportError / AttributeError in TDD mode (correct).
...
class TestEndpointSurface:
    ...
    def test_app_has_exactly_three_routes(self) -> None:
        ...
        WHY: In TDD mode, app.routes contains only GET /health (the Chunk 1
        stub). This test MUST fail until the router is mounted. After Chunk 3,
        the route count must be exactly 3: POST /events/ingest, POST /events/bulk,
        GET /health (replaced, not additive). Any extra route violates spec §7.
        """
...
class TestPublishFailureResilience:
    ...
    The IngestionService._safe_publish already swallows publish failures (Chunk 2).
    ...
    def test_single_ingest_returns_202_when_service_swallows_publish_failure(
        ...
        # guarantees per spec §5.5 / Chunk 2).
        fake = FakeIngestionService(one_returns=_KNOWN_UUID_1)
        _, dep_callable = _import_app_and_service_dep()
        if dep_callable is None:
            pytest.fail("get_ingestion_service dependency not found")
