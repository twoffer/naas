# Component: NAAS Spec 1 — Chunk 2: app/adapters.py
# Mode: TDD — all tests MUST fail until the implementer creates:
#   services/event-ingestion/app/adapters.py
#
# What these tests validate:
#
#   PostgresEventRepository._to_orm mapping (spec §§5.3, 3.1):
#     - _to_orm(record) returns an EventORM instance
#     - id, user_id, protocol, client_ip, timestamp, source, is_synthetic,
#       is_historical, raw_attributes are all copied from the LoginEventRecord
#     - normalized_attributes, enriched_signals, created_at are left unset
#       (None or not present on the ORM object) — ingestion does not populate them
#
#   PostgresEventRepository.persist / persist_many (spec §5.3):
#     - persist(record) calls session.add() and session.commit() exactly once each
#     - persist_many(records) calls session.add_all() once and session.commit() once
#
#   RedisEventPublisher.publish (spec §§5.3, 3.2):
#     - publish(record) calls publish_to_stream with:
#         arg[0] == STREAM_LOGIN_EVENTS
#         arg[1] is a dict produced by record.model_dump(mode="json")
#         arg[1]["id"] is a string (UUIDs must be serialized as strings for JSON)
#
# Assumption about _to_orm:
#   The spec §5.3 shows '_to_orm(record)' as a module-level helper used inside
#   PostgresEventRepository. We test the mapping behavior via:
#   1. A direct call to PostgresEventRepository._to_orm(record) if it is a
#      static/class method or module-level function accessible via the class;
#   2. Falling back to observing what session.add() receives in persist().
#   Both approaches are correct per the spec's intent. We prefer #1 as a direct
#   unit test of the mapping contract; we include #2 as a behavioral fallback.
#   The implementation may name this _to_orm on the class (static) or at module level.
#
# sys.path strategy:
#   Same injection as all chunk tests in this directory.
