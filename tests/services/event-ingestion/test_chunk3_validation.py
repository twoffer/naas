# Component: NAAS Spec 1 — Chunk 3: validation rejection tests for routes
# Mode: TDD — ALL tests in this file MUST FAIL until the implementer creates:
#   services/event-ingestion/app/routes.py
#   services/event-ingestion/app/main.py (modified to mount router)
#
# What these tests validate (spec §§2.1, 2.2, 3.3, 5.6):
#   (a) Bulk size: empty array → 422; array length > 5000 → 422. In both cases
#       the service's ingest_many must NOT have been called (rejection happens
#       at FastAPI request-validation time, before the handler executes).
#   (b) Field validation: client_ip = "256.0.0.1" → 422 (octet out of range);
#       protocol = "kerberos" → 422 (not a valid Literal). In both cases
#       ingest_one must NOT have been called.
#
# Design rationale for the 422 without-service-call assertion:
#   FastAPI validates the request body (via Pydantic) BEFORE calling the handler.
#   If validation fails, the handler is never invoked. We install a FakeIngestionService
#   via dependency_overrides and assert its call counts remain 0 after a 422 response.
#   This verifies that nothing is written even when the request body is malformed.
#
# The >5000 bulk test uses a list of 5001 minimal valid event dicts. The bodies are
# intentionally minimal — the exact content past the size limit is irrelevant.
# Constructing 5001 dicts in a test is unavoidable for a behavioral boundary test.
#
...
def _get_dep_callable():
    ...
    Returns None if routes.py does not exist yet (TDD initial state).
    In that case, dependency_overrides cannot be set and the test will fail
    for a different reason (no route exists → 404 or ImportError) — both
    are valid TDD-failure modes.
    """
    import importlib
