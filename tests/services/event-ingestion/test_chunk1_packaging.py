# Component: NAAS Spec 1 — Chunk 1: packaging artifacts
...
#   docker-compose.yml (event-ingestion entry added)
#
# What these tests validate:
#   requirements.txt:
#     - Lists fastapi (any version pin)
#     - Lists uvicorn (any version pin, with or without [standard])
#     - Does NOT list sqlalchemy, asyncpg, or redis (these come via naas_shared)
#
#   Dockerfile:
#     - File exists at services/event-ingestion/Dockerfile
#     - Contains EXPOSE 8001
#     - COPYs shared/ BEFORE service code (layer caching contract)
#     - Contains `pip install -e /app/shared/` (or equivalent editable install)
#     - CMD launches uvicorn on app.main:app port 8001
#
#   .dockerignore:
#     - Exists at repo root
#     - Contains entries for: .git, .venv, node_modules, .claude, **/__pycache__
#
#   docker-compose.yml:
#     - Parses as valid YAML
#     - event-ingestion service is present
#     - build.context == '.'
#     - build.dockerfile == 'services/event-ingestion/Dockerfile'
#     - env_file == '.env' (or is a list containing '.env')
#     - Port mapping to container port 8001
#     - depends_on: postgres with condition: service_healthy
#     - depends_on: redis with condition: service_healthy
#     - healthcheck is present
#     - All four infrastructure services (postgres, redis, keycloak, openldap) remain present
#
# Why this matters:
#   These files are the operational contract for the event-ingestion service. A wrong
#   Dockerfile context or COPY order breaks the image build. Missing depends_on conditions
#   cause the service to start before the DB/Redis are ready, causing startup crashes.
#   env_file absence means POSTGRES_*/REDIS_* env vars are never injected.
#   The .dockerignore keeps build contexts lean — without it, .git and node_modules
#   are included in every service's build context, slowing all future builds.
#
# TDD state:
#   Dockerfile and requirements.txt do not exist yet.
#   .dockerignore does not exist yet.
#   docker-compose.yml exists but has no event-ingestion entry.
#   All tests MUST fail until the implementer creates the files.
