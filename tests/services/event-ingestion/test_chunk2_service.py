# Component: NAAS Spec 1 — Chunk 2: app/service.py (IngestionService dual-write)
# Mode: TDD — all tests MUST fail until the implementer creates:
#   services/event-ingestion/app/service.py
#
# What these tests validate (spec §§5.4, 5.5):
#   (a) ingest_one(event) returns a UUID equal to the persisted record's id,
#       and calls repo.persist exactly once BEFORE publisher.publish (ordering).
#   (b) When the publisher raises, ingest_one STILL returns the id (no exception
#       propagates) and the injected logger.error is called once with
#       event_id=str(record.id).
#   (c) ingest_many([e1,e2,e3]) calls repo.persist_many exactly once with a
#       3-element list and returns the three ids in input order.
#   (d) When the publisher raises on every record in ingest_many, publish is
#       still attempted for all three records (best-effort) and no exception propagates.
#
# Design rationale for fakes vs. mocks:
#   We use hand-rolled fake classes (FakeRepo, FakePublisher) rather than MagicMock
#   because ordering assertions need a shared call log. A call log (a plain list
#   appended to by both fakes) lets us assert that 'persist' precedes 'publish'
#   without relying on mock call order inspection, which is fragile.
#
# FakeRepo and FakePublisher satisfy the EventRepository / EventPublisher Protocols
#   structurally — they have the same async method signatures. No inheritance needed.
#
# sys.path strategy:
...
class TestIngestionServiceImport:
    ...
    WHY: ModuleNotFoundError here is the expected TDD state before implementation.
    After implementation, this must pass without error.
    """
