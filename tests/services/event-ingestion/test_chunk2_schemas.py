# Component: NAAS Spec 1 — Chunk 2: app/schemas.py (API response models)
# Mode: TDD — all tests MUST fail until the implementer creates:
#   services/event-ingestion/app/schemas.py
#
# What these tests validate:
#   - `from app.schemas import IngestAccepted, BulkIngestAccepted` resolves
#   - IngestAccepted(id=<uuid>).status == "accepted" (Literal default)
#   - BulkIngestAccepted(accepted=2, event_ids=[<uuid>,<uuid>]).status == "accepted"
#   - app.schemas does NOT expose ORM symbols (EventORM, Base) — it is the API
#     response module, not the ORM module (shared/naas_shared/schemas.py has those)
#   - Both models are Pydantic BaseModel subclasses
#   - Field types match the spec §5.6 contract (UUID, int, list[UUID], Literal)
#
# WHY app.schemas must not contain ORM symbols:
#   The spec §1 explicitly warns: 'app/schemas.py (API response models) and
#   shared/naas_shared/schemas.py (ORM table definitions) are different files
#   with the same basename. Keep them distinct.' Mixing them would create circular
#   import risks and expose SQLAlchemy internals via the HTTP response layer.
#
# sys.path strategy:
...
class TestSchemasModuleDoesNotExposeORMSymbols:
    ...
    def test_app_schemas_source_does_not_import_event_orm(self) -> None:
        ...
        assert schemas_path.exists(), (
            f"app/schemas.py does not exist yet at {schemas_path} — expected in TDD state."
        )
