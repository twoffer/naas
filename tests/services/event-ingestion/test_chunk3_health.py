# Component: NAAS Spec 1 — Chunk 3: GET /health with real PG + Redis probes
# Mode: TDD — some tests MAY PASS against the Chunk 1 /health stub (noted inline).
#             The degraded and unhealthy tests MUST FAIL against the stub because
#             the stub always returns "healthy" without checking dependencies.
#
# What these tests validate (spec §5.6):
#   (a) Both PG and Redis OK → 200 with status="healthy", service="event-ingestion".
#   (b) Redis ping fails but PG OK → 200 with status="degraded".
#   (c) PG check fails → 200 with status="unhealthy".
#   (d) HTTP status code is ALWAYS 200 regardless of health status (spec §5.6
#       states the HTTP status is always 200; the operational status is in the body).
#   (e) Response body conforms to the shared HealthResponse model in all three cases.
#
# Override strategy:
#   We mock the DB session's execute (for 'SELECT 1') and the Redis ping. The
#   exact override target depends on the implementer's wiring:
#
#   ASSUMPTION A — the implementer wires /health via FastAPI dependency_overrides
#   using naas_shared.database.get_db_session and naas_shared.redis_client.get_redis.
#   We override both at the module level (patch at the naas_shared namespace).
#
#   ASSUMPTION B — the implementer calls `session.execute(text("SELECT 1"))` inside
#   the /health handler (using the injected session). We mock execute() on the
#   AsyncMock session to succeed or raise as needed.
#
#   ASSUMPTION C — the implementer calls `await (await get_redis()).ping()` inside
#   the /health handler. We patch get_redis() to return a mock whose ping() raises
#   or returns normally.
#
#   If the implementer uses a different wiring (e.g., a separate health dependency),
#   the patcher targets below need to be updated. The behavioral contract (status string
#   in the body) is what we assert — the mock targets are documented so the implementer
#   knows what to match.
#
# NOTE: Test class TestHealthStatusHealthy tests the "both OK → healthy" scenario.
#   This scenario will PASS against the Chunk 1 skeleton because the stub always
#   returns "healthy". That is EXPECTED and noted in the docstrings.
#   Tests for "degraded" and "unhealthy" will FAIL against the Chunk 1 stub.
#   The endpoint-surface tests (test_chunk3_routes.py) also ensure the old stub
#   is replaced (since it only has GET /health with no event routes).
#
# sys.path strategy: identical to prior chunk tests.
...
#
# NOTE: Tests in this class will PASS against the Chunk 1 stub because the stub
# always returns "healthy". That is the expected and documented TDD state.
# The chunk 3 tests that MUST fail are in TestHealthStatusDegraded and
# TestHealthStatusUnhealthy below.
# ===========================================================================
...
class TestHealthStatusHealthy:
    ...
    NOTE: These tests will PASS against the Chunk 1 skeleton (which returns a
    static healthy response without checking PG/Redis). That is EXPECTED and
    is not a TDD violation — the happy-path behavior is identical between the
    stub and the real implementation.
    """
...
#
# NOTE: These tests MUST FAIL against the Chunk 1 stub, which always returns
# "healthy" regardless of Redis state. That is the expected TDD initial state.
# ===========================================================================
...
class TestHealthStatusDegraded:
    ...
    MUST FAIL AGAINST CHUNK 1 STUB: The stub returns "healthy" unconditionally.
    The degraded tests will fail because the stub ignores the mocked Redis failure.
    """
    ...
    def test_health_status_is_degraded_when_redis_fails_and_pg_ok(self) -> None:
        ...
        WHY: Spec §5.6 — 'Redis down but PG OK → "degraded"'. This is the key
        behavioral contract this test enforces. The Chunk 1 stub returns "healthy"
        here — this test MUST FAIL against the stub and pass only after Chunk 3
        implements the real PG + Redis connectivity checks.
        """
        from starlette.testclient import TestClient
        from app.main import app
        ...
        assert body.get("status") == "degraded", (
            ...
            "This test MUST fail against the Chunk 1 stub (which returns 'healthy')."
        )
...
#
# NOTE: These tests MUST FAIL against the Chunk 1 stub, which always returns
# "healthy". They define the contract for real PG connectivity checking.
# ===========================================================================
...
class TestHealthStatusUnhealthy:
    ...
    MUST FAIL AGAINST CHUNK 1 STUB: stub always returns "healthy".
    """
    ...
    def test_health_status_is_unhealthy_when_pg_fails(self) -> None:
        ...
        health state: no new events can be accepted at all. The Chunk 1 stub
        returns "healthy" here — this test MUST FAIL against the stub.
        """
        from starlette.testclient import TestClient
        from app.main import app
