...
class TestLdapAdapterProtocolConformance:
    ...
    def test_ldap_adapter_has_enrich_method(self) -> None:
        ...
        WHY: The method must be defined so that the composition root can wire
        LdapAdapter as the LdapEnricher without AttributeError at startup.
        """
        from app.adapters.ldap import LdapAdapter
...
class TestLdapAdapterBareStringMemberOf:
    ...
    Behavior: a bare-string memberOf must yield [] via coerce_str_list (no DN
    reduction attempted), never be iterated character-by-character.
    """
