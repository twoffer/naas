...
class TestValidationErrorLogRedaction:
    ...
    async def test_validation_error_log_does_not_contain_pii_email(self) -> None:
        ...
        # A ValidationError at parse time means the message was never processed;
        # it must NOT be XACKed — the message stays in the pending-entries list.
        assert not redis.xack.called, (
            "XACK must NOT be called when the stream message fails LoginEventRecord "
            "validation.  A message that cannot be deserialized was never processed "
            "and must remain in the pending-entries list for manual inspection."
        )
