# Component: NAAS Spec 2 — Chunk 1: ports.py Protocol definitions
# Mode: TDD — all tests MUST fail until the implementer creates:
#   services/identity-normalization/app/ports.py
#
# What these tests validate:
#   - app.ports imports without error
#   - ProtocolAdapter is defined as a typing.Protocol and is runtime-checkable
#   - LdapEnricher is defined as a typing.Protocol and is runtime-checkable
#   - NormalizationRepository is defined as a typing.Protocol and is runtime-checkable
#   - EventPublisher is defined as a typing.Protocol and is runtime-checkable
#   - Each Protocol defines the expected method signatures (inspected via typing)
#
# WHY ports.py matters (hexagonal architecture):
#   Spec §5 and ADR-0009 mandate a hexagonal (ports/adapters) structure.
#   Port Protocols define the I/O boundary contracts for the domain core.
#   The NormalizationService (domain) depends on these abstractions, not on
#   concrete implementations. This enables test doubles and future adapter swaps.
#   If ports.py doesn't exist or its Protocol methods are missing, the service
#   cannot be wired, and the domain tests (later chunks) cannot use mock adapters.
#
# TDD state:
#   services/identity-normalization/app/ports.py does not exist yet.
#   All tests MUST fail with ModuleNotFoundError or AttributeError.
...
def _find_repo_root() -> Path:
    ...
    raise RuntimeError(
        "Could not locate repo root. Started from: {Path(__file__).resolve()}"
    )
...
class TestPortsModuleImport:
    ...
    in later-chunk tests.
    """

    def test_ports_module_is_importable(self) -> None:
        """from app.ports import ... must succeed without raising.

        WHY: ModuleNotFoundError here means services/identity-normalization/app/ports.py
        does not exist yet — correct TDD initial state.
        """
...
class TestProtocolsAreRuntimeCheckable:
    ...
    def test_normalization_repository_is_a_protocol(self) -> None:
        ...
        structurally. Tests in later chunks use a mock that satisfies the Protocol
