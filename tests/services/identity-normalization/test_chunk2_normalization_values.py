# Component: NAAS Spec 2 — Chunk 2: normalization_values.py
...
#   - DEPARTMENT_CANONICAL dict: transcribed verbatim from spec §5.2
#   - EMPLOYEE_TYPE_CANONICAL dict: transcribed verbatim from spec §5.2
#   - UNIFIED_TO_LDAP reverse-map: {unified_field: ldap_attribute} (§5.2 mapping table)
#   - normalize_department(value) -> (str, bool): hit→(canonical,True); miss→(title(),False)
#   - normalize_employee_type(value) -> str|None: hit→canonical string; miss→None (never raw)
#   - Case-insensitivity and whitespace stripping on both helpers
#   - Canonical target strings are byte-identical (cross-protocol equality)
#
# WHY this module matters:
#   normalization_values.py is the single source of truth for canonical value
#   mappings. All three protocol adapters call normalize_department() and
#   normalize_employee_type() so that OIDC "eng", SAML "eng", and LDAP "eng"
#   all resolve to the exact string "Engineering". Cross-protocol conflict
#   resolution (§5.5) depends on value-normalized strings being equal.
#   UNIFIED_TO_LDAP is reused by the LDAP enrichment adapter (§5.3) to
#   reverse-map unified field names to LDAP attribute names at runtime.
#
# TDD state:
#   services/identity-normalization/app/normalization_values.py does NOT exist yet.
...
class TestNormalizationValuesImport:
    ...
    def test_module_is_importable(self) -> None:
        ...
        WHY: ModuleNotFoundError here is the correct TDD initial failure state.
