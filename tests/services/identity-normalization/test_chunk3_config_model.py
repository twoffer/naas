# Component: NAAS Spec 2 — Chunk 3: normalization_config.py
...
#   - NormalizationConfig is importable and instantiable via load_config()
#   - Pydantic model structure reflects the §5.6 YAML schema
#   - Accessor helpers: weight_for(), priority_for(), merge_strategy_for()
#   - Fallback behaviour when an attribute has no explicit entry in the YAML
#   - Fallback behaviour when a source is not in an attribute's explicit weights block
#   - groups attribute has no weights block → falls back to defaults.source_weights
#   - priority_for() returns [] when no priority configured for attribute
#   - merge_strategy_for() returns "union" default when no strategy configured
#
# WHY this module matters:
#   normalization_config.py is the authoritative runtime source of per-attribute
#   authority weights and enrichment settings. Every conflict resolution in §5.5
#   calls weight_for() to determine source confidence. An incorrect weight silently
#   changes the winning source in every priority-resolution, which directly affects
#   which identity attributes reach the Risk Evaluator and Dashboard.
#
# TDD state:
...
#   All tests MUST fail with ModuleNotFoundError or FileNotFoundError.
