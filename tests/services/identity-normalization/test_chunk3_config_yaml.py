# Component: NAAS Spec 2 — Chunk 3: config/normalization.yaml round-trip
...
#   - config/normalization.yaml exists and is parseable by load_config()
#   - load_config() of the committed file does NOT raise (file is itself valid)
#   - Per-attribute weights from §5.6 YAML match exactly (transcribed values)
#   - Per-attribute priority lists from §5.6 YAML match exactly
#   - groups merge_strategy is "union" as configured
#   - Enrichment block scalar values match §5.6 exactly
#   - Fallback: weight_for('groups', 'ldap') == 0.7 (groups has no weights block)
#   - The committed YAML is its own spec-compliance proof
#
# WHY testing the committed YAML matters:
#   §5.6 labels the YAML values [TRANSCRIBE EXACTLY]. The implementer must produce
#   a YAML file that matches these values byte-for-byte. If they transpose two weights
#   (e.g. department ldap=0.70 instead of 0.90), the resolution engine promotes the
#   wrong source in every department conflict — silently, without a test failure.
#   This file is the spec-compliance proof for the committed config artifact.
#
# TDD state:
#   config/normalization.yaml does NOT exist yet.
#   app/normalization_config.py does NOT exist yet.
#   Tests fail with FileNotFoundError and ModuleNotFoundError respectively.
#   After implementation, both must be resolved for these tests to pass.
...
class TestCommittedYamlFileExists:
    ...
    WHY: The implementer must create config/normalization.yaml as part of this
    chunk. If the file is missing, the service cannot start (§5.1: 'loads and
