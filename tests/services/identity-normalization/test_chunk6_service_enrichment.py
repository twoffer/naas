"""Identity Normalization Service — Chunk 6: NormalizationService.normalize()
Component: services/identity-normalization/app/service.py (NOT YET CREATED)
Mode: TDD — ALL tests MUST fail until app/service.py is implemented.
...
SEAM / SIGNATURE ASSUMPTIONS (implementer must conform):
  ...
  class NormalizationService:
      def __init__(
          ...
          ldap_adapter: LdapAdapter,   # satisfies both ProtocolAdapter + LdapEnricher ports
      ) -> None: ...

      async def normalize(self, record: LoginEventRecord) -> NormalizedAttributes: ...

  The constructor receives pre-built adapter instances (dependency injection).
  The service calls:
    - adapter.extract(record.raw_attributes) for the primary protocol
    - ldap_adapter.enrich(correlation_field, lookup_value) → (attrs, outcome)
  The LdapAdapter.enrich() 2-tuple is patched in every test below by mocking
  ldap_adapter.enrich directly (patch the instance method via monkeypatch or
  unittest.mock.AsyncMock).

ENRICHMENT DECISION RULE (§5.4):
  - Attempt enrichment IFF: ldap_enabled AND protocol in ("oidc", "saml")
  - NEVER branch on is_synthetic.  Real and synthetic events are treated identically.
  - The correlation_value is the primary-attrs value for config.enrichment.sources.ldap.correlation_key
    (defaults to "primary_email").
  - Missing/empty correlation value → EnrichmentSkipped(skip_reason="invalid_correlation_key"),
    enrich() is NOT called.

OUTCOME → skip_reason MAPPING (§5.4 closed enum, also see models.py EnrichmentSkipReason):
  "cache_hit_positive"    → EnrichmentApplied(applied=True, source="ldap", cache_hit=True)
  "cache_hit_negative"    → EnrichmentSkipped(skip_reason="no_ldap_match")
  "ldap_match"            → EnrichmentApplied(applied=True, source="ldap", cache_hit=False)
  "ldap_no_match"         → EnrichmentSkipped(skip_reason="no_ldap_match")
  "ldap_timeout"          → EnrichmentSkipped(skip_reason="ldap_timeout")
  "ldap_connection_error" → EnrichmentSkipped(skip_reason="ldap_connection_error")
  "ldap_search_error"     → EnrichmentSkipped(skip_reason="ldap_search_error")
  "ldap_unexpected_error" → EnrichmentSkipped(skip_reason="ldap_search_error")  [folded into catch-all]
  "unmappable_field"      → EnrichmentSkipped(skip_reason="ldap_search_error")  [folded into catch-all]
    NOTE: "unmappable_field" and "ldap_unexpected_error" have no dedicated Literal in models.py.
    Both are folded into "ldap_search_error" (the "Other LDAP-side error" row in §5.4).
    This interpretation is stated explicitly so the implementer conforms.
"""
