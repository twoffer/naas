# Component: NAAS Spec 2 — Adapter refactor: app/adapters/_mapping.py
# Mode: TDD — all tests in this file MUST FAIL until the implementer creates:
#   services/identity-normalization/app/adapters/_mapping.py
#
# What these tests validate:
#   - coerce_str: passthrough for str; None for every non-str type
#   - coerce_str_list: list-of-str passthrough; mixed-type filtering;
#     empty list; None → []; STRICT: bare string "admin" → [] (not char list)
#   - FieldRule: NamedTuple with .source_keys and .transform accessors
#   - apply_field_rules: single-key rules; multi-key rules; missing keys;
#     output dict key ordering matches rule declaration order
#
# WHY this module matters:
#   _mapping.py is the shared engine that each protocol adapter's FIELD_RULES
#   table runs through.  It is the single place where:
#     - Source-claim → value coercion logic lives (coerce_str, coerce_str_list)
#     - Multi-source merging syntax is defined (FieldRule with positional args)
#   If coerce_str_list iterates a bare string character-by-character (the easy
#   mistake), policy conditions like `"admin" in groups` silently fire for any
#   user who has a single-string groups claim — a privilege escalation vector.
#
# TDD state:
#   app/adapters/_mapping.py does NOT exist yet.
#   All tests here MUST fail with ModuleNotFoundError (or ImportError) until
#   the implementer creates the module.  The adapter classes already exist;
#   only the _mapping import will fail.
...
class TestMappingModuleImport:
    ...
    def test_mapping_module_is_importable(self) -> None:
        ...
        WHY: ModuleNotFoundError is the correct TDD initial failure state.
        After implementation this must succeed.
        """
