"""Identity Normalization Service — Chunk 6: Publisher (publish_normalized)
Component: services/identity-normalization/app/service.py or a dedicated publisher class
Mode: TDD — ALL tests MUST fail until the publisher is implemented.

SEAM / SIGNATURE ASSUMPTIONS (implementer must conform):
  The publisher satisfies the EventPublisher port in app/ports.py:
  ...
  The concrete implementation (NormalizationPublisher or similar) must:
    1. Set record.normalized_attributes = normalized.model_dump(mode="json")
    2. Call publish_to_stream(STREAM_NORMALIZED_EVENTS, record.model_dump(mode="json"))
       — the FULL LoginEventRecord with normalized_attributes populated
    3. Use the shared constant STREAM_NORMALIZED_EVENTS (never a hardcoded string)
    4. Use the shared publish_to_stream helper (never a hand-rolled XADD)

  The publisher class is expected to be importable as:
    from app.service import NormalizationPublisher
  OR
    from app.publisher import NormalizationPublisher
  (Implementer chooses; tests patch naas_shared.redis_client.publish_to_stream
   and verify the observable behavior regardless of class location.)
"""
