# Component: NAAS Spec 2 — Chunk 5: resolution.py — overall normalization_confidence
...
#
# WHAT THESE TESTS VALIDATE (§5.5.2 confidence contract):
#   1. ATTRIBUTE_IMPORTANCE constants are exactly as specified in §5.5.2.
#   2. normalization_confidence = sum(IMPORTANCE[a] × per_attr_conf.get(a, 0.0)) clamped [0,1].
#   3. All-absent → 0.0 overall confidence.
#   4. All-present unanimous → weighted sum of per-attribute max weights.
#   5. Partial presence → absent attributes contribute 0.0 (not 1.0 or any default).
#   6. Overall confidence is clamped to [0.0, 1.0].
#   7. §3.3 representative example: hand-computed expected value.
#
# ATTRIBUTE_IMPORTANCE (§5.5.2 [TRANSCRIBE EXACTLY]):
#   display_name:   0.15
#   primary_email:  0.25
#   department:     0.20
#   employee_type:  0.25
...
#   Risk Evaluator computes normalization_risk = 1.0 - normalization_confidence.
#   An inflated confidence suppresses the risk signal.  An incorrect importance
#   weighting changes which attributes dominate the risk score.
