# Component: NAAS Spec 2 — Chunk 1: docker-compose.yml identity-normalization entry
# Mode: TDD — all tests MUST fail until the implementer adds the identity-normalization
#   service entry to docker-compose.yml
#
# What these tests validate (spec §5.8):
#   - identity-normalization service entry is present in docker-compose.yml
#   - build.context == '.' (repo root — required for COPY shared/ + service code)
#   - build.dockerfile == 'services/identity-normalization/Dockerfile'
#   - env_file == '.env' or list containing '.env'
#   - Port mapping to container port 8002 (${IDENTITY_NORMALIZATION_PORT:-8002}:8002)
#   - depends_on: postgres with condition: service_healthy
#   - depends_on: redis with condition: service_healthy
#   - depends_on: openldap with condition: service_healthy (NEW vs event-ingestion)
#   - config bind-mount: ./config:/app/config (read-only)
#   - healthcheck present with /health probe on port 8002
#   - All existing services (postgres, redis, keycloak, openldap, event-ingestion)
#     remain byte-unchanged (presence check via YAML parse)
#
# openldap dependency (critical for this service):
#   The identity-normalization service queries OpenLDAP for LDAP enrichment.
#   Unlike event-ingestion (which has no openldap dependency), this service
#   must wait for openldap to be healthy before starting, otherwise the first
#   OIDC/SAML event that triggers enrichment will fail to connect. The
#   condition: service_healthy ensures OpenLDAP's healthcheck (ldapsearch) has
#   passed before the normalization service starts its consumer loop.
#
# config bind-mount (critical):
#   config/normalization.yaml is loaded at startup (spec §5.1). The mount
#   ./config:/app/config makes this file available inside the container at
#   /app/config/normalization.yaml. Without the mount, startup fails because
#   the config file is missing. It must be read-only so the service cannot
#   accidentally modify the config source.
#
# Scope preservation:
#   Adding the identity-normalization entry must NOT modify or remove any
#   existing service. This test suite verifies that all five previously-existing
#   services (postgres, redis, keycloak, openldap, event-ingestion) are still
#   present after the change.
#
# TDD state:
#   docker-compose.yml exists but has no identity-normalization entry.
#   All tests MUST fail with assertion errors until the entry is added.
...
class TestExistingServicesUnchanged:
    ...
    def test_all_six_services_present(self) -> None:
        """docker-compose.yml must contain all 6 expected services after this chunk.
