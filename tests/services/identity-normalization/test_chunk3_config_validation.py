# Component: NAAS Spec 2 — Chunk 3: normalization_config.py — startup validation
...
#   - load_config() raises on invalid correlation_key (not in UNIFIED_TO_LDAP)
#   - The raised error message names the offending key (descriptive error)
#   - The rejection uses UNIFIED_TO_LDAP as the single source of truth (no hardcoded copy)
#   - load_config() raises on invalid on_failure value (not "continue" or "fail")
#   - load_config() raises on cache_ttl_seconds == 0
#   - load_config() raises on cache_ttl_seconds < 0
#   - load_config() raises on enrich_attributes containing a name outside UNIFIED_TO_LDAP
#   - load_config() does NOT raise on valid enrich_attributes (subset of unified fields)
#   - load_config() does NOT raise on omitted enrich_attributes (optional key)
#   - load_config() does NOT raise on all valid on_failure values ("continue", "fail")
#
# WHY startup validation matters for IAM:
#   §5.6 states: "invalid config aborts startup with a descriptive error."
#   An invalid correlation_key (e.g. 'favorite_color') would cause the LDAP
#   enrichment adapter to attempt a reverse-map on an unknown unified field,
#   silently returning None and disabling enrichment for ALL events. This is a
#   silent correctness failure that would only surface as enrichment never being
#   applied — an insidious security regression. Catching it at startup prevents
#   silent partial failures during operation.
#
# WHY UNIFIED_TO_LDAP is the single source of truth:
#   The implementer must NOT hardcode a second copy of the valid unified-field set
#   inside normalization_config.py. Two copies will inevitably drift apart.
#   The tests in TestCorrelationKeyValidation.test_*_uses_unified_to_ldap_keys
#   assert this linkage explicitly: any field in UNIFIED_TO_LDAP must be valid,
#   and any field NOT in UNIFIED_TO_LDAP must be rejected.
#
# TDD state:
#   services/identity-normalization/app/normalization_config.py does NOT exist yet.
