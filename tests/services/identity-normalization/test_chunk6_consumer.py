"""Identity Normalization Service — Chunk 6: Consumer loop (consumer.py)
Component: services/identity-normalization/app/consumer.py (NOT YET CREATED)
Mode: TDD — ALL tests MUST fail until app/consumer.py is implemented.
...
SEAM / SIGNATURE ASSUMPTIONS (implementer must conform):
  ...
  async def run_consumer_loop(
      ...
      # Runs indefinitely; processes messages from login_events stream
      # using XREADGROUP group=normalization_workers consumer=<hostname-based name>

  Alternatively, a class-based consumer:
  class NormalizationConsumer:
      def __init__(self, service, repository, publisher, redis) -> None: ...
      async def run(self) -> None: ...

  The critical observable behaviors tested here:
  1. Message parse: LoginEventRecord.model_validate(json.loads(fields["data"]))
  2. Ordering: service.normalize → repository.write(commit) → publisher.publish_normalized → XACK
  3. No XACK on exception
  4. Enrichment failure does NOT cause drop/unACK
  5. DB session obtained from get_session_factory(), not request-scoped get_db_session
  6. Consumer name derived from hostname
  7. Reads via XREADGROUP with {STREAM_LOGIN_EVENTS: ">"}, group GROUP_NORMALIZATION

The consumer loop entrypoint (function name) is assumed to be either:
  - run_consumer_loop (function) at module level, or
  - NormalizationConsumer.run() (method)
Both are tested via the same mock-based approach: we mock one iteration by
controlling what XREADGROUP returns (one message then KeyboardInterrupt to break).
"""
