# Component: NAAS Spec 2 — Chunk 5: resolution.py — normalization-failure 0.2 penalty
...
#
# WHAT THESE TESTS VALIDATE (§5.5 normalization-failure penalty contract):
#
# The 0.2 penalty rule (§5.5):
#   'The 0.2 penalty attaches to a resolution's confidence ONLY when the resolved
#   (winning) value is itself an unmapped value (clamped to [0.0, 1.0]).
#   This can happen ONLY for department, whose unmapped values are retained (§5.2);
#   it can NEVER happen for employee_type, whose unmapped values are discarded to None.'
#
# was_mapped encoding in attribute_sources:
#   department tuple: (normalized_str, was_mapped: bool)
#     was_mapped=True  → the value was found in DEPARTMENT_CANONICAL → NO penalty
#     was_mapped=False → the value was not in DEPARTMENT_CANONICAL, retained title-cased → penalty
#
# Test matrix:
#   1. SingleSource dept, was_mapped=True  → no penalty (full weight)
#   2. SingleSource dept, was_mapped=False → penalty: confidence = weight - 0.2
#   3. Unanimous dept,  all_mapped=True    → no penalty (max weight)
#   4. Unanimous dept,  winner unmapped    → Unanimous with penalty? No — unanimous means
#                                            all agree. If they all agree on an unmapped value,
#                                            the "winning" value IS unmapped → -0.2 applies.
#   5. Priority dept,   winner mapped       → no penalty (0.72 full priority confidence)
#   6. Priority dept,   winner unmapped     → penalty: 0.72 - 0.2 = 0.52
#   7. employee_type:   NEVER carries penalty regardless of input (unmapped discarded upstream)
#   8. Discarded employee_type source does NOT penalize surviving valid source
#   9. Penalty clamps at 0.0 (confidence cannot go negative)
#   10. Other scalar attrs (display_name, primary_email) — no was_mapped tuple,
#       no penalty mechanism — full weight always.
#
# ENCODED INTERPRETATION on the unanimous-with-unmapped case:
#   If all sources agree on an unmapped department value, the resolution type is
#   UnanimousResolution (because they agree), but the −0.2 penalty still applies
#   because the winning/resolved value is unmapped.  The penalty is about the
#   VALUE, not the resolution type.  This is consistent with §5.5: 'attaches to
#   a resolution's confidence only when the resolved (winning) value is itself an
#   unmapped value'.
#
# WHY this matters for security:
#   The penalty is the primary mechanism for surfacing data-quality issues to the
#   Risk Evaluator (via normalization_risk = 1 - confidence).  A missing penalty
#   on an unmapped department means a user's department affiliation is misclassified
#   but the Risk Evaluator sees it as high-confidence, suppressing the risk signal.
