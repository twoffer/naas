"""Identity Normalization Service — Chunk 6: EnrichmentSkipped/Applied skip_reason mapping.
...
OUTCOME → ENRICHMENT VARIANT MAPPING (§5.4, models.py EnrichmentSkipReason):
  ...
  "ldap_unexpected_error" → EnrichmentSkipped(skip_reason="ldap_search_error")  [no dedicated Literal]
  "unmappable_field"      → EnrichmentSkipped(skip_reason="ldap_search_error")  [no dedicated Literal]

  Config disabled: EnrichmentSkipped(skip_reason="ldap_disabled")
  Protocol="ldap": EnrichmentSkipped(skip_reason="ldap_event")
  Missing correlation value: EnrichmentSkipped(skip_reason="invalid_correlation_key")

GRACEFUL DEGRADATION INVARIANT (§5.4, ADR-0008):
  On ANY skip or failure, normalize() STILL returns a valid NormalizedAttributes.
  It NEVER raises. The result carries primary-source-only data.
"""
