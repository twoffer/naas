# Component: NAAS Spec 2 — Chunk 5: resolution.py — scalar attribute resolution
...
#       ...
#
# ATTRIBUTE_SOURCES SHAPE:
#   A dict mapping each unified attribute name to the sources that supplied a
#   non-null, normalized value for it.  The value for each source is:
#
#     For "department": a tuple (normalized_str: str, was_mapped: bool)
#       where was_mapped=True means normalize_department() found the key in
#       DEPARTMENT_CANONICAL, was_mapped=False means it used the title-cased
#       retained fallback.  Example:
#         {"department": {"ldap": ("Engineering", True), "oidc": ("Product", False)}}
#
#     For all other scalar attributes ("display_name", "primary_email",
#       "employee_type"): a plain str (already normalized by the adapter,
#       None values excluded).  Example:
#         {"display_name": {"oidc": "Alice Smith", "ldap": "Alice Smith"},
#          "primary_email": {"oidc": "alice@corp.com"},
#          "employee_type": {"ldap": "FTE"}}
#
#     For "groups": a list[str] of normalized group names per source.  Example:
#         {"groups": {"oidc": ["admin", "vpn-users"], "ldap": ["engineering"]}}
#
#   Attributes absent from attribute_sources (or present with an empty sub-dict)
#   have 0 contributing sources → unified value is None, 0.0 confidence,
#   no entry in resolution_details.
#
#   IMPORTANT: Only sources with NON-NULL, NON-EMPTY values appear as keys inside
#   the per-attribute sub-dict.  The caller (NormalizationService) is responsible
#   for filtering out None/empty values before calling resolve().
#
# WHAT THESE TESTS VALIDATE (§5.5 scalar contract):
#   1. Zero sources → unified None, no resolution_details entry, 0.0 contrib to confidence.
#   2. Single source → SingleSourceResolution with correct confidence = weight_for(attr, src).
#   3. ≥2 sources agreeing → UnanimousResolution with confidence = max(weights), correct sources.
#   4. ≥2 sources disagreeing → PriorityResolution: winner per priority_for(), confidence =
#      winner_weight × 0.8, conflicting_values has non-null losers, penalty_applied=True.
#   5. source_protocol on output equals the passed primary protocol even with ldap enrichment.
#   6. enrichment on output is the exact variant passed in (not computed by resolve).
#   7. returned object is a valid NormalizedAttributes (Pydantic validates it).
#
# WHY these matter:
#   Incorrect resolution directly changes which identity attributes (email, department,
#   employee_type) reach the Risk Evaluator and Dashboard.  A transposed priority
#   list or wrong confidence formula is a silent security defect.
...
class TestResolutionModuleImport:
    ...
    The failing state here is a ModuleNotFoundError — the correct TDD initial state.
    """

    def test_resolution_module_is_importable(self) -> None:
        """from app.resolution import resolve must not raise.

        WHY: ModuleNotFoundError is the expected TDD initial failure.
        Once the implementer creates resolution.py this passes.
        """
