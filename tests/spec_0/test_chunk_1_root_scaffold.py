# Component: NAAS Spec 0 — Chunk 1: Root scaffolding and service directory tree
# Mode: TDD — all tests MUST fail until the chunk is implemented
#
# What these tests validate:
#   - .env.example exists and contains every required key=value pair
#   - .env exists and is byte-for-byte identical to .env.example
#   - All eight services/<name>/README.md files exist and contain the required phrase
#   - Each services/<name>/ directory contains ONLY README.md (no extra files)
#   - config/ and scripts/ directories exist but do NOT yet contain their later-phase files
#   - .gitignore contains the five required lines
#   - README.md retains the 'Normalized Adaptive Access System' tagline
#
# Why this matters:
#   The scaffold chunk establishes the structural contract that every downstream spec
#   assumes. Wrong port values, missing env vars, or extra files inside services/*/
#   cause cascading failures in Specs 1-6. These tests are the gate.
...
class TestDotEnvFile:
    ...
    def test_dot_env_exists(self, env_path):
        ...
        Chunk 1 ships .env pre-populated from .env.example.
        """
...
class TestServiceReadmeFiles:
    ...
    The phrase confirms that the README was created intentionally by this chunk
    ...
    def test_service_readme_contains_required_phrase(self, service_name):
        ...
        phrase that marks the file as intentionally scaffolded by Chunk 1.
        """
        readme = REPO_ROOT / "services" / service_name / "README.md"
...
class TestConfigAndScriptsDirs:
    """
    config/ and scripts/ are scaffolded in Chunk 1 (via .gitkeep placeholders)
    but their real content (normalization.yaml, train_bootstrap_model.py) is
    created by later specs.  These tests enforce that boundary: the directories
    must exist but the later-phase files must NOT yet be present.
    """

    def test_config_directory_exists(self):
        """config/ must exist (created via config/.gitkeep in Chunk 1)."""
    ...
    def test_scripts_directory_exists(self):
        """scripts/ must exist (created via scripts/.gitkeep in Chunk 1)."""
    ...
    def test_train_bootstrap_model_does_not_exist(self):
        """
        scripts/train_bootstrap_model.py must NOT exist in Chunk 1.
        ...
        assert not bootstrap_script.exists(), (
            f"scripts/train_bootstrap_model.py must not exist in Chunk 1 — "
...
class TestRootReadme:
    """
    README.md is a shared file that predates Chunk 1.  Its tagline must be
    preserved.  Any implementation that silently overwrites README.md would
    break this test.
    """
