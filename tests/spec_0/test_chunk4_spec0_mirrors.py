# Component: NAAS Spec 2 — Chunk 4: SPEC_0 documentation mirror parity
# Mode: TDD — all tests MUST fail until the implementer adds the two mirror lines to:
#   docs/architecture/SPEC_0_Project_Scaffold_and_Shared_Foundation.md
#     §3.3 — add: LDAP_ENRICHMENT_CACHE_PREFIX = "ldap_enrichment:"
#     §3.8 — add: ldap_pool_size: int = Field(default=3, ge=1, le=10)
#
# What these tests validate:
#   1. SPEC_0 §3.3 (Constants block) contains the LDAP cache prefix line
#   2. SPEC_0 §3.8 (Shared Config block) contains the ldap_pool_size field line
#   3. Both lines appear in the expected sections (after the relevant section headers)
#   4. The exact strings match — no typos, no wrong defaults
#
# Why this matters (from spec §1 [CRITICAL]):
#   "A shared module and its SPEC_0 mirror change together or not at all; deferring
#    the doc update is exactly how the spec and the code drift out of sync."
#   These tests enforce that the SPEC_0 mirror IS updated in this same pipeline run.
#   Without these tests, a pipeline run that passes all code tests but skips the
#   doc update would silently create spec/code drift — the canonical documentation
#   mirror would describe a Settings class with no pool_size field while the
#   implementation has one, making SPEC_0 unreliable as a reference.
#
# TDD state:
#   The SPEC_0 doc does NOT yet contain LDAP_ENRICHMENT_CACHE_PREFIX or ldap_pool_size.
#   Both tests MUST fail with AssertionError until the implementer adds the mirror lines.
...
class TestSpec0Section33CachePrefixMirror:
    ...
    def test_spec0_contains_ldap_enrichment_cache_prefix_string(self) -> None:
        ...
        This is the exact line that §1 mandates be added to §3.3. A missing or
        misspelled line fails this test with a clear AssertionError.

        TDD: Fails until the implementer adds the mirror line.
        """
        content = _read_spec0()
...
class TestSpec0Section38LdapPoolSizeMirror:
    ...
    def test_spec0_contains_ldap_pool_size_field_line(self) -> None:
        ...
        This is the exact line that §1 mandates be added to §3.8. The exact
        string includes the Field constraints so readers understand the validation
        rules directly from the spec without having to read the source code.
