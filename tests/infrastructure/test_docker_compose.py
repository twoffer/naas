"""docker-compose.yml service definitions, networking, volumes, and healthchecks."""
...
# Source files (relative to repo root) that the compose stack depends on
# existing. The first three are bind-mounted at runtime; the openldap
...
def _load_compose() -> dict[str, Any]:
    ...
    if not COMPOSE_FILE.exists():
        pytest.fail(
            ...
            "This file is required by the compose stack."
        )
...
def compose() -> dict[str, Any]:
    ...
    Fails fast if the file is absent or unparseable.
    """
...
class TestComposeFileExistence:
    ...
    def test_docker_compose_file_exists_at_repo_root(self) -> None:
        ...
        means the compose stack has not been initialized.
        """
    ...
    def test_docker_compose_validates_via_docker_compose_config(self) -> None:
        ...
        while the file is absent it will FAIL (not skip), by design.
        """
...
class TestRequiredSourceFilesExist:
    """Assert that each compose source file (bind-mounted or Dockerfile COPY) is
    present on disk.

    NOTE: These assertions may PASS even before docker-compose.yml is created
    because the source files come from the infrastructure/service definitions.
    This is acceptable — the suite as a whole still fails due to the missing
    compose file.
    """
    ...
    def test_required_source_file_exists(self, rel_path: str) -> None:
        """Each compose source file must exist on disk.
        ...
        Files validated:
          - infrastructure/postgres/init.sql                (bind-mount)
          - infrastructure/redis/redis.conf                 (bind-mount)
          - infrastructure/keycloak/naas-realm-export.json  (bind-mount)
          - infrastructure/openldap/bootstrap.ldif          (Dockerfile COPY)
        """
