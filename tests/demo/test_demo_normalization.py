...
# lock generated from it (see DEPENDENCIES.md).
DEMO_REQUIREMENTS_IN = DEMO_DIR / "requirements.in"
...
def requirements_text() -> str:
    """Raw text of demo/requirements.txt (the compiled lock)."""
...
@pytest.fixture(scope="module")
def requirements_in_text() -> str:
    """Raw text of demo/requirements.in (the declared floors)."""
    if not DEMO_REQUIREMENTS_IN.exists():
        pytest.fail(f"demo/requirements.in not found at {DEMO_REQUIREMENTS_IN}")
    return DEMO_REQUIREMENTS_IN.read_text(encoding="utf-8")
...
class TestRequirementsTxt:
    """Demo deps: floors declared in requirements.in (rich, httpx, psycopg[binary]),
    pinned closure generated into the requirements.txt lock."""

    def test_requirements_in_exists(self) -> None:
        """demo/requirements.in (the declared floors) must exist."""
        assert DEMO_REQUIREMENTS_IN.exists(), (
            f"demo/requirements.in not found at {DEMO_REQUIREMENTS_IN}"
        )

    def test_requirements_file_exists(self) -> None:
        """demo/requirements.txt (the compiled lock README installs) must exist."""
        assert DEMO_REQUIREMENTS.exists(), (
            f"demo/requirements.txt not found at {DEMO_REQUIREMENTS}"
        )

    def test_requirements_txt_is_compiled_lock(self, requirements_text: str) -> None:
        """requirements.txt must be a pip-compiled lock, not a hand-edited list.

        WHY: The reproducibility posture (ADR-0012) requires the install artifact
        to be the generated lock; the pip-compile header marks it as such and
        records how to regenerate it.
        """
        assert "autogenerated by pip-compile" in requirements_text, (
            "demo/requirements.txt must be generated by pip-compile (its header "
            "should say so). Regenerate it from demo/requirements.in — see "
            "DEPENDENCIES.md."
        )

    def test_rich_is_listed(self, requirements_in_text: str) -> None:
        """requirements.in must declare rich with a pinned version."""
        assert re.search(r"^rich[>=<!\[]", requirements_in_text, re.MULTILINE), (
            f"'rich' with a version pin not found in requirements.in:\n{requirements_in_text}"
        )

    def test_httpx_is_listed(self, requirements_in_text: str) -> None:
        """requirements.in must declare httpx with a pinned version."""
        assert re.search(r"^httpx[>=<!\[]", requirements_in_text, re.MULTILINE), (
            f"'httpx' with a version pin not found in requirements.in:\n{requirements_in_text}"
        )

    def test_psycopg_binary_is_listed(self, requirements_in_text: str) -> None:
        """requirements.in must declare psycopg[binary] with a pinned version.

        The floor declares the [binary] extra; the compiled lock expands it to the
        underlying psycopg + psycopg-binary distributions (pip-compile --strip-extras).
        """
        # Accept psycopg[binary]==... or psycopg[binary]>=... etc.
        assert re.search(r"^psycopg\[binary\]", requirements_in_text, re.MULTILINE), (
            f"'psycopg[binary]' not found in requirements.in:\n{requirements_in_text}"
        )
