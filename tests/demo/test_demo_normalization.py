...
class TestCliHelp:
    ...
    EXPECTED_FLAGS = ["--keep", "--pace", "--step", "--timeout", "--skip-verify", "--ingest-url", "--db-dsn"]
    ...
    def test_help_lists_all_seven_flags(self) -> None:
        """--help output must contain exactly the seven documented flags (all present)."""
        result = self._run_help()
        combined = result.stdout + result.stderr
        missing = [f for f in self.EXPECTED_FLAGS if f not in combined]
        assert not missing, (
            f"Flags missing from --help output: {missing}\nOutput: {combined}"
        )

    def test_pace_default_is_1_5(self) -> None:
        """--help output must show 1.5 as the default for --pace."""
        result = self._run_help()
        combined = result.stdout + result.stderr
        assert "1.5" in combined, (
            f"Default 1.5 for --pace not found in --help output.\nOutput: {combined}"
        )

    def test_timeout_default_is_30(self) -> None:
        """--help output must show 30 as the default for --timeout."""
        result = self._run_help()
        combined = result.stdout + result.stderr
        assert "30" in combined, (
            f"Default 30 for --timeout not found in --help output.\nOutput: {combined}"
        )
...
class TestScenesConstant:
    ...
    def test_scene_has_required_keys(self, demo_module, idx: int) -> None:
        ...
        scene = demo_module.SCENES[idx]
        required = {"user_id", "protocol", "client_ip", "source", "is_synthetic", "raw_attributes"}
        missing = required - set(scene.keys())
        assert not missing, (
            f"SCENES[{idx}] is missing keys: {missing}"
        )
    ...
    @pytest.mark.parametrize("idx", range(6))
    def test_scene_client_ip_is_documentation_range(self, demo_module, idx: int) -> None:
...
class TestScenesIdentity:
    ...
    @pytest.mark.parametrize(
        "idx, user_id, protocol, client_ip",
        [
    ...
    def test_scene_user_id(self, demo_module, idx: int, user_id: str, protocol: str, client_ip: str) -> None:
        """Scene at index idx must have the correct user_id."""
        scene = demo_module.SCENES[idx]
    ...
    def test_scene_protocol(self, demo_module, idx: int, user_id: str, protocol: str, client_ip: str) -> None:
        """Scene at index idx must have the correct protocol."""
        scene = demo_module.SCENES[idx]
    ...
    def test_scene_client_ip(self, demo_module, idx: int, user_id: str, protocol: str, client_ip: str) -> None:
        """Scene at index idx must have the correct client_ip."""
        scene = demo_module.SCENES[idx]
...
class TestScenesRawAttributes:
    ...
    def test_scene0_frank_oidc_raw_attributes(self, demo_module) -> None:
        ...
        assert ra.get("employee_type") == "E", f"Got employee_type={ra.get('employee_type')!r}"
    ...
    def test_scene1_frank_saml_raw_attributes(self, demo_module) -> None:
        ...
        assert ra.get("displayName") == "Frank Castle", f"Got displayName={ra.get('displayName')!r}"
        assert ra.get("email") == "frank@corp.com", f"Got email={ra.get('email')!r}"
        assert ra.get("dept") == "Engineering", f"Got dept={ra.get('dept')!r}"
        assert ra.get("employeeType") == "FTE", f"Got employeeType={ra.get('employeeType')!r}"
        assert ra.get("groups") == ["engineering", "vpn-users"], (
            f"Got groups={ra.get('groups')!r}"
        )
    ...
    def test_scene2_grace_ldap_raw_attributes(self, demo_module) -> None:
        ...
        assert ra.get("departmentNumber") == "r&d", f"Got departmentNumber={ra.get('departmentNumber')!r}"
        assert ra.get("employeeType") == "C", f"Got employeeType={ra.get('employeeType')!r}"
        expected_member_of = [
    ...
    def test_scene3_mallory_saml_raw_attributes(self, demo_module) -> None:
        ...
        assert ra.get("displayName") == "Mallory Quinn", f"Got displayName={ra.get('displayName')!r}"
        assert ra.get("email") == "mallory@corp.com", f"Got email={ra.get('email')!r}"
        assert ra.get("dept") == "Sorcery", f"Got dept={ra.get('dept')!r}"
        assert ra.get("employeeType") == "wizard", f"Got employeeType={ra.get('employeeType')!r}"
        assert ra.get("groups") == ["temp-access"], (
            f"Got groups={ra.get('groups')!r}"
        )
    ...
    def test_scene4_alice_oidc_raw_attributes(self, demo_module) -> None:
        ...
        assert ra.get("department") == "eng", f"Got department={ra.get('department')!r}"
        assert ra.get("employee_type") == "FTE", f"Got employee_type={ra.get('employee_type')!r}"
...
class TestNaasSharedSoftImport:
    """naas_shared import must be wrapped in try/except so absence does not raise."""

    def test_naas_shared_import_is_wrapped_in_try_except(self, demo_source: str) -> None:
        """Any 'from naas_shared' or 'import naas_shared' line must be inside a try/except block.

        Strategy: walk the AST and locate every Import/ImportFrom node whose
        module starts with 'naas_shared'.  For each such node, verify it sits
        inside an ExceptHandler (i.e. is reachable only inside a try/except).
        """
        tree = ast.parse(demo_source)

        # Build a mapping: node -> parent for every node in the tree.
        parent_map: dict[ast.AST, ast.AST] = {}
        for node in ast.walk(tree):
            for child in ast.iter_child_nodes(node):
                parent_map[child] = node

        def _is_inside_try_except(node: ast.AST) -> bool:
            """Return True if node has a Try ancestor (i.e. is guarded)."""
        ...
        naas_imports: list[ast.AST] = []
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                if (node.module or "").startswith("naas_shared"):
                    naas_imports.append(node)
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("naas_shared"):
                        naas_imports.append(node)
                        break

        assert naas_imports, (
            "demo_normalization.py must contain at least one naas_shared import "
            "(wrapped in try/except)"
        )
        for imp in naas_imports:
            assert _is_inside_try_except(imp), (
                f"naas_shared import at line {getattr(imp, 'lineno', '?')} "
                "must be wrapped in a try/except block so its absence does not raise"
            )

    def test_module_defines_fallback_flag_or_plain_dict_path(self, demo_source: str) -> None:
        """Module must define a fallback variable or use a plain-dict path when naas_shared is absent.

        Structural check: the source contains either a boolean flag set in the
        except branch (e.g. NAAS_SHARED_AVAILABLE = False) or a plain-dict
        alternative, evidenced by a try/except block that contains a naas_shared
        import and an except body that is not just 'pass' with no assignment.
        """
        tree = ast.parse(demo_source)

        # Collect all Try nodes that guard a naas_shared import.
        guarding_tries: list[ast.Try] = []
        for node in ast.walk(tree):
            if isinstance(node, ast.Try):
                for child in ast.walk(node):
                    if isinstance(child, ast.ImportFrom) and (child.module or "").startswith("naas_shared"):
                        guarding_tries.append(node)
                        break
        ...
        assert guarding_tries, (
            "Expected at least one Try block that guards a naas_shared import"
        )

        # Each guarding try must have at least one handler whose body is
        # non-trivial (not solely a bare 'pass' with zero assignments).
        for try_node in guarding_tries:
            for handler in try_node.handlers:
                body_stmts = handler.body
                non_pass = [s for s in body_stmts if not isinstance(s, ast.Pass)]
                if non_pass:
                    # At least one non-pass statement in some handler — good.
                    return

        pytest.fail(
            "The except branch of the naas_shared try/except must contain at least one "
            "non-pass statement (e.g. a fallback flag assignment) to establish the plain-dict path"
        )
...
class TestRequirementsTxt:
    ...
    def test_naas_shared_not_listed(self, requirements_text: str) -> None:
        ...
        naas_lines = [ln for ln in lines if ln.startswith("naas_shared") or ln.startswith("naas-shared")]
...
class TestReadme:
    ...
    def test_readme_mentions_flags(self, readme_text: str) -> None:
        ...
        flag_mentions = sum(
            1 for flag in ["--keep", "--pace", "--step", "--timeout", "--skip-verify", "--ingest-url", "--db-dsn"]
            if flag in readme_text
        )
        assert flag_mentions > 0, (
            "README must document the CLI flags"
        )

    def test_readme_mentions_postgres_direct_read_honesty_note(self, readme_text: str) -> None:
        ...
        has_direct = bool(re.search(r"direct(ly)?|reads?\s+[Pp]ostgres|PostgreSQL\s+direct", readme_text, re.IGNORECASE))
...
class TestNoMetaLanguage:
    ...
    def _grep_files(self) -> list[tuple[str, str, int]]:
        """Return list of (filepath, token, line_number) for every banned-token hit."""
            ...
            for lineno, line in enumerate(text.splitlines(), start=1):
                for token in self.BANNED_TOKENS:
                    if token.lower() in line.lower():
                        hits.append((str(fpath), token, lineno))
        return hits
