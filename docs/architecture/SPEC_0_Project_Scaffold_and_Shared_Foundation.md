...
- No `shared/naas_shared/ml_features.py` content (placeholder module; the 16-feature column contract is defined and created by Spec 3)
- No `shared/naas_shared/simulation_tools.py` content (placeholder module; tool definitions land with the persona-simulator spec)
...
# Keycloak (H2 dev-mode database — no Postgres dependency; see SPEC_0 §3.1 Architect's Note)
