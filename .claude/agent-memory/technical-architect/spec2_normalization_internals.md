...
**`shared/pyproject.toml` already exists** (naas-shared build config; no `[tool.pytest.ini_options]`). Any new pytest config (e.g. pytest-timeout) goes in a NEW *root* `pyproject.toml`, never in `shared/` — putting an inifile in `shared/` could drag pytest rootdir into `shared/` and break the root `conftest.py` `pytest_collect_file` + per-service `pytest_runtest_setup` sys.modules juggling. A `[tool.pytest.ini_options]` table at repo root pins rootdir to repo root, which is where it already resolves — no-op for discovery.
