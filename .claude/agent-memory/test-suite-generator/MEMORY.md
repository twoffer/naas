...
- [patterns_dual_write_failure_tests.md](patterns_dual_write_failure_tests.md) — FailingRepo idiom, fake isolation between test files, route-level 5xx pattern, call_log ordering assertions, Redis Stream XREVRANGE search fixture, bulk URL fixture
- [patterns_spec2_review_remediation.md](patterns_spec2_review_remediation.md) — Canonical conftest fake consolidation, sys.path cleanup strategy (retained _REPO refs), E402/F401 remediation, exact-equality guards for spec-pinned dicts, round-trip invariant tests, poison-message no-ACK, TestLdapAttrMergeIntoResolution with real config
