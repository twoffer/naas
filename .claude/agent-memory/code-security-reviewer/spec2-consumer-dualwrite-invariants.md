...
## Drift hazard (chunk-6) — RESOLVED post-spec-2
- `service.py` previously defined DEAD `_MATCH_OUTCOMES` (frozenset) and `_OUTCOME_TO_SKIP_REASON` (dict) at module top (defs only; the real mapping was inline in `_map_outcome_to_enrichment`) — two copies of the §5.4 table that could silently diverge. The unused constants were DELETED in the post-spec-2 remediation (fix G); the inline mapping in `_map_outcome_to_enrichment` is now the single source. Watch for this pattern re-emerging in new outcome→skip_reason changes.
