...
- [Dependency pinning posture](dependency-pinning-posture.md) — ADR-0012 floors(.in/pyproject)→compiled(.txt) locks, `--no-deps` shared install, CI drift job, Dependabot lockfile-only, iter_routes (FastAPI≥0.137), no-hash supply-chain gap (accepted)
