...
- If a chunk creates or modifies an installable package (e.g. `shared/`), `pip install -e shared/ --no-deps` it (its runtime deps come from the dev lockfile) before running that chunk's tests.
