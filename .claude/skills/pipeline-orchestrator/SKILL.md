...
5. **Budget guard.** Increment `invocation_count` in `state.json` after every `Agent` tool call. If `invocation_count >= 30` and `budget_guard_triggered` is `false`, pause and report status to the developer via `AskUserQuestion` before continuing, then set `budget_guard_triggered = true` in `state.json`. The guard fires exactly once per run — the flag prevents it from re-firing as `invocation_count` keeps climbing past 30. Append a §5.2.11 (`AWAITING INPUT`) bullet using the `Budget guard` reason from CONTRACTS.md §5.3, and on resume append a §5.2.12 (`RESUMED`) bullet using the `Budget-guard continuation` decision from §5.4. The guard is evaluated immediately after the increment and **before** processing the worker's result. So when a single invocation both reaches the threshold and produces a failure-driven escalation (rule 6), handle the budget guard first: emit its ⏸/▶ pair, then the failure escalation's ⏸/▶ pair.
