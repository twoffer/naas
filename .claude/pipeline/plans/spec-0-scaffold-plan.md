...
- Downstream consumers of this spec's artifacts:
  * Every future service imports `naas_shared` (Chunk 2) and, per spec §4 / Areas-of-Care #5, MUST mount `./shared:/app/shared` and `RUN pip install -e /app/shared/` in its Dockerfile. Forgetting the mount is the spec's #1 predicted failure mode (ModuleNotFoundError: naas_shared).
